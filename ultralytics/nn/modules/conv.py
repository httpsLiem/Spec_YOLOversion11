...
class RecycleConv(nn.Module):
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.s = s
        self.conv = nn.Conv2d(c1 // 8, self.c_, k, s, autopad(k, p, d), groups=g, dilation=d, bias=False)
        self.bn_silu = nn.Sequential(nn.BatchNorm2d(self.c_), nn.SiLU())
...
class LDConv1(nn.Module):
    def __init__(self, inc, outc, num_param, stride=1, bias=None):
        ...
        self.stride = stride
        self.conv = nn.Sequential(
            nn.Conv2d(inc, outc, kernel_size=(1, num_param), stride=(1, num_param), bias=bias),
            nn.BatchNorm2d(outc),
            nn.SiLU(),
        )  # the conv adds the BN and SiLU to compare original Conv in YOLOv5.
        self.p_conv = nn.Conv2d(inc, 2 * num_param, kernel_size=3, padding=1, stride=stride)
...
class MaxConvHCA(nn.Module):  # MSCBam
    ...
    def forward(self, x):
        ...
        #                                     padding=(self.kz // 2, 0))
        x1 = torch.nn.functional.max_pool2d(x, kernel_size=(1, self.kz), stride=self.stride, padding=(0, self.kz // 2))
        # print('gc')
        # x1 = self.msc(x1)
        # x2 = torch.nn.functional.max_pool2d(x, kernel_size=self.kz, stride=2)
        # x3 = torch.cat((x1, x2),1)
...
class PartialConvMax(nn.Module):
    def __init__(self, c1, c2, k=3, s=2, d=1, act=True):
        ...
        self.fc = nn.Sequential(
            nn.Conv2d(c1 * 2, 8, 1, 1, 0, bias=True),
            # nn.BatchNorm2d(8),
            nn.ReLU(),
        )
...
class SobelSpatialAttention(nn.Module):
    ...
    def forward(self, x):
        """Apply channel and spatial attention on input for feature recalibration."""
        return x * self.act(
            self.cv1(self.sobel(torch.cat([torch.mean(x, 1, keepdim=True), torch.max(x, 1, keepdim=True)[0]], 1)))
        )
...
class SAKAM1(nn.Module):
    ...
    def __init__(self, c1, kernel_size=7):
        ...
        self.up = nn.Sequential(nn.Conv2d(int(c1 / 4), c1, 1, 1, 0, bias=True), nn.Sigmoid())
...
class Fusion(nn.Module):
    def __init__(self, inc_list, fusion="bifpn", c1=128) -> None:
        ...
        if self.fusion == "bifpn":
            ...
            self.fc1 = nn.Sequential(nn.Conv2d(8, c1, kernel_size=1, stride=1, padding=0), nn.ReLU())
            # self.act = nn.Sigmoid()
...
class Faster_Block(nn.Module):
    def __init__(
        ...
        self.spatial_mixing = Partial_conv3(dim, n_div, pconv_fw_type)  # PConv or call spatial_mixing

...
#             # Hai lớp ConvGN tiền kỳ
#             self.pre_conv1 = nn.ModuleList(ConvGN(x, x, 1) for x in ch)
#             self.pre_conv2 = nn.ModuleList(ConvGN(x, x, 1) for x in ch)  # Thêm lớp pre_conv2
#             #self.se_layers = nn.ModuleList(SELayer(x) for x in ch)
...
#                 else nn.ModuleList(
#                     nn.Sequential(
#                         nn.Sequential(DWConv(x, x, 3), ConvGN(x, c3, 1)),
#                         nn.Conv2d(c3, self.nc, 1),
#                     )
#                     for x in ch
#                 )
...
#             for i in range(self.nl):
#                 # Áp dụng cả hai lớp pre-conv
#                 x[i] = self.pre_conv1[i](x[i])
#                 #x[i] = self.se_layers[i](x[i])
#                 x[i] = self.pre_conv2[i](x[i])  # Thêm lớp pre_conv2
#                 x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
#             if self.training:
#                 return x
#             y = self._inference(x)
...
#                 x[i] = self.pre_conv1[i](x[i])
#                 #x[i] = self.se_layers[i](x[i])
#                 x[i] = self.pre_conv2[i](x[i])  # Thêm lớp pre_conv2 cho end2end
#                 x[i] = torch.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
#             if self.training:
#                 return {"one2many": x, "one2one": one2one}
